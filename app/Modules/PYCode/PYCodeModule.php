...
namespace App\Modules\PYCode;


use App\Models\BaseModel;

class PYCodeModule extends BaseModel
{
    /**
    ...
    public function getfirstchar($s0)
    {
        ...
        if (($asc >= -12838 && $asc <= -12557) || (in_array($asc, array(-22842)))) return 'W';
...
//    }
    ...
    public function getFullSpell($chinese, $delimiter = '', $length = 0)
    {
        ...
        $return_data = array(
            'py_code' => strtoupper($spell),//全拼音
            'py_initials' => '',//首字母
        );
        for ($i = 0; $i < mb_strlen($spell); $i++) {
            if (!(ord($spell[$i]) >= ord('a') && ord($spell[$i]) <= ord('z'))) {
                $return_data['py_initials'] .= $spell[$i];
            }
        }
        return $return_data;
    }
