...
class OrderComplainsModel extends BaseModel {
    ...
    public function queryByPage($page=1,$pageSize=15){
        $shopName = WSTAddslashes(I('shopName'));
	 	...
	 	$sql = "select oc.complainId,o.orderId,o.orderNo,s.shopName,u.userName,u.loginName,oc.createTime,oc.complainStatus,oc.complainType
	  ...
	  */
		...
		if($rs['complainStatus']!=4 and $isPay==1){


				$mod_order_complains = M('order_complains')->where("complainId = {$id}")->find();
				$mod_order = M('orders')->where("orderId = {$rs['orderId']}")->find();
				...
				$pay_logUserId = $rs['userId'];
				$pay_transaction_id = $mod_order['tradeNo'];
				$pay_total_fee = $mod_order['realTotalMoney'] * 100;
				$pay_refund_fee = goodsPayN($mod_order_complains['orderId'],$mod_order_complains['goodsId'],$mod_order_complains['skuId'],$mod_order_complains['complainTargetId']) * 100;
				...
				if($ispayOK == true){//如果退款成功 更改投诉状态
					...
					$this->finalResult = '总部已同意！允许退款';
					$this->finalHandleStaffId = 1;
					$this->complainStatus = 4;


					$rsr = $this->where('complainId='.$id)->save();
					if($rsr !==false){
						$rd['status'] = 1;
					}else{
						$rd['msg'] = '提交订单应诉信息失败';
				...
				if($rs['orderStatus']==-5){
					...
					if($orderStatus==-4 && $rs["useScore"]>0){//同意用户拒收
						...
						if($rs["poundageMoney"]>0){
							...
							$data["dataSrc"] = 1;
							$data["dataId"] = $orderId;
							$data["moneyRemark"] = "收取订单【".$rs["orderNo"]."】".$rs[""]."%的佣金 ￥".$rs["poundageMoney"];
							$data["moneyType"] = 1;
