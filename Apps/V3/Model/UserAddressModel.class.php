...
class UserAddressModel extends BaseModel {
        ...
        if($this->checkEmpty($data,true)){
            ...
            if(false !== $rs){
                ...
                if((int)I("isDefault")==1){
                    //修改所有的地址为非默认
                    $m->isDefault = 0;
                    $m->where('userId='.(int)$userId." and addressId!=".$rs)->save();
                }
            }
        }
        return $rd;
    ...
    public function edit($userId){
        ...
        if($this->checkEmpty($data,true)){
            ...
            $rs = $m->where("userId=".(int)$userId." and addressId=".$id)->save($data);
            if(false !== $rs){
                $rd['status']= 1;
                if((int)I("isDefault")==1){
                    //修改所有的地址为非默认
                    ...
                    $m->where('userId='.(int)$userId." and addressId!=".$id)->save();
                }
            }
        }
        return $rd;
    ...
    public function get($userId){
        ...
        return $m->where("addressId=".(int)I('id')." and userId=".(int)$userId)->find();
    }
    ...
    public function del($userId){
        ...
        $rs = $m->where("userId=".(int)$userId." and addressId=".(int)I('id'))->delete();
        if(false !== $rs){
            $rd['status']= 1;
        }
        return $rd;
    ...
    public function getUserAddressInfo($userId){
        ...
        */
        $sql ="SELECT ad.*, a.areaName FROM __PREFIX__user_address ad
			  left join __PREFIX__areas a ON ad.areaId2 = a.areaId
		      WHERE ad.addressId=$addressId AND ad.addressFlag=1 and ad.userId=".(int)$userId;
     ...
     * 获取地址详情
     */
    public function getAddressDetails($addressId, $userId){
        ...
        $sql ="SELECT * FROM __PREFIX__user_address WHERE addressId=$addressId AND addressFlag=1 and userId=".(int)$userId;
