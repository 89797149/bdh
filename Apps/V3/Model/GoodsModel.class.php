...
//        $sql = "SELECT img.* FROM __PREFIX__goods_gallerys img WHERE img.goodsId = $goodsId ";
    ...
    public function insert($data){
        ...
        if ($m->create()){
            ...
            $m->firstDistribution = (int)I("firstDistribution",0);//一级分销
            $m->SecondaryDistribution = (int)I("SecondaryDistribution",0);//二级分销
    ...
    public function get($parameter){
        ...
        $goods['gallery'] = $m->where($whereGallery)->select();
     ...
     * 批量修改商品状态
     */
    public function goodsSet($parameter = array()){
        $rd = array('status'=>-1);
    ...
    public function getGoodsInfo($goodsId,$goodsAttrId){
        $sql = "SELECT g.attrCatId,g.goodsId,g.goodsName,g.goodsStock,g.bookQuantity,g.isBook,g.isSale,g.goodsImg,g.isNewPeople,g.goodsThums,g.goodsSpec,g.markIcon,g.goodsStock FROM __PREFIX__goods g WHERE g.goodsId = $goodsId AND g.goodsFlag = 1 AND g.goodsStatus = 1";
        $rs = $this->queryRow($sql);
        if(!empty($rs) && $rs['attrCatId']>0){
            $sql = "select ga.id,ga.attrPrice,ga.attrStock,a.attrName,ga.attrVal,ga.attrId from __PREFIX__attributes a,__PREFIX__goods_attributes ga
			        where a.attrId=ga.attrId and a.catId=".$rs['attrCatId']."
...
//        $goodsStatus = (int)I('goodsStatus');
        $goodsName = WSTAddslashes(I('goodsName'));
        $sql = "select g.goodsId,g.shopId,g.SuppPriceDiff,g.goodsSn,g.shopGoodsSort,g.goodsName,g.goodsImg,g.goodsThums,g.shopPrice,g.goodsStock,g.saleCount,g.isSale,g.isRecomm,g.isHot,g.isBest,g.isNew,ga.isRecomm as attIsRecomm,g.saleTime,g.createTime from __PREFIX__goods g
		left join __PREFIX__goods_attributes ga on g.goodsId = ga.goodsId and ga.isRecomm = 1
		where g.goodsFlag=1
		 and g.shopId=".$shopId;
        if($shopCatId1>0)$sql.=" and g.shopCatId1=".$shopCatId1;
