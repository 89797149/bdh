...
class AttributeCatsModel extends BaseModel { 
    /**
	  * 新增
	  */
	 public function insert($parameter=array()){
	 	$m = M('attribute_cats');
	 	$rd = array('status'=>-1);
		$data = array();
		$data['catName'] = I('catName');
		$data["shopId"] = (int)session('WST_USER.shopId');
		$data["shopId"] = $data["shopId"]?$data["shopId"]:$parameter['shopId'];
	  ...
	  * 修改
	  */
	 public function edit($parameter=array()){
	 	$m = M('attribute_cats');
	 	$rd = array('status'=>-1);
	  ...
	  * 分页列表
	  */
     public function queryByList($data=array()){
     	 $m = M('attribute_cats');
     	 $shopId = (int)session('WST_USER.shopId');
		if(empty($shopId)){
		    $shopId = $this->MemberVeri()['shopId'];
	  ...
	  * 删除
	  */
	 public function del($parameter= array()){
	    $rd = array('status'=>-1);
	    $id = (int)I('id');
	    if($id==0)return $rd;
	    $m = M('attributes');
