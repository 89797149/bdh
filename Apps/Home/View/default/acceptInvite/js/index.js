$(document).ready(function () {
	...
	$('.submit').click(function () {
		...
		var phoneVal = $(".phone[name='phone']").val();
		var regPhone =/^1\d{10}$/;
		...
		$.ajax({
			url: "//"+window.location.host+'/v3/index/acceptInvite',
			type: 'post',
			data: {
				'inviterId': userId,
				'phone': phoneVal,
				'mobileCode': verifyVal
			},
			dataType: 'json',
			beforeSend: function () {},
			complete: function () {},
	...
	$('.getVerificationCode').click(function () {
		...
		$.ajax({
			url: "//"+window.location.host+'/v3/Index/getPhoneInviteVerify',
			type: 'post',
			data: {
				'userPhone': phoneVal
			},
