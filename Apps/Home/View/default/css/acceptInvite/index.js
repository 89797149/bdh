$(document).ready(function () {
	...
	$('.submit').click(function () {
		...
		$.ajax({
			url: 'https://www.qinghucun.net/Weimendian/Index/distributionLog',
			type: 'post',
			data: {
				'userId': userId,
				'userPhone': phoneVal,
			...
			success: function (res) {
				if (res.apiCode == 1) {
					...
					setTimeout(() => {
						window.location.href = 'file:///D:/fenxiao/download';
					}, 3000);
	...
	$('.getVerificationCode').click(function () {
		...
		$.ajax({
			url: 'https://www.qinghucun.net/Weimendian/Index/distributionPhoneVerify',
			type: 'post',
			data: {
				'userPhone': phoneVal
			},
