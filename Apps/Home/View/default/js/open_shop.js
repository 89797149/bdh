function visitorShopInit(){
		...
		$("#deliveryType").formValidator({onFocus:"请选择配送方式"}).inputValidator({min:1,onError: "请选择配送方式"});
...
function userShopInit(){
   ...
   $.formValidator.initConfig({
	...
	$("#deliveryType").formValidator({onShow:"",onFocus:"请选择配送方式",onCorrect:"已选择"}).inputValidator({min:1,onError:"请选择配送方式"});
